...
module Load_dir_status = struct
  ...
  (* the Starting case seems to be useless at the moment - however it might help
     checking correctness of load_dir *)
  type t =
    | Starting of Starting.t
    | Running of Running.t
end
...
(* this is the actual execution of the scheme *)
...
(* the files that need to wait_for_file before loading the scheme *)
...
(* the directories that need to be completely loaded before the scheme *)
...
(* the directories that need to be completely loaded before loading the rule -
   the directories mentioned in Build.Paths_glob and Build.If_file_exists *)
let rule_dir_deps rule =
  let { Pre_rule.build; _ } = rule in
  Build_interpret.dir_deps build

(* check that evaluating globs and if_file_exists again would not change the rule *)
